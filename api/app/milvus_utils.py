...
def insert_embeddings_in_milvus(
    ...
    del words, num_words
    if ran_batching:
        del text, num_text, model_output, ran_batching
...
def insert_pca_embeddings_in_milvus(
    ...
    for batch in tqdm(
        ...
        if num_text != batch_size: # If last batch doesn't match the normal batch_size
            response = embedding_collection.query(
                expr=f"id in {list(range(batch, batch + num_text))}",
                output_fields=["embedding"],
                consistency_level="Strong"
            )
        else:
            response = embedding_collection.query(
                expr=f"id in {list(range(batch, end_batch))}",
                output_fields=["embedding"],
                consistency_level="Strong"
            )
    ...
    # free some memory
    del words, num_words, transformed_embeddings
